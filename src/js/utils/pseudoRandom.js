...
const chanceBetween = ( chance=50, funcOne, funcTwo, num=43) =>
{
  ...
  function checkArrays()
  {
    if(yourArr.length === max)
    {
      for(let el of array)
      {
        if(yourArr[num] === el)
        {
          console.log(`Было найдено совпадения элементов\nСтарое число: ${num}`);
...
const historyRandom = (num, min=0, max=100, array, n=3, dOaF=1, pseudoRandom=false) =>
{
  ...
  function check()
  {
    for(let i of array)
    {
      ...
      {   
        if( !pseudoRandom ) num = Math.round(Math.random(100-0) + 0)
